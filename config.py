...
from strictyaml import as_document, Map, Bool, Regex, load as yaml_load
...
__CONFIG_SCHEMA = Map({
    ...
    'main': Map({'channel': _WORD, 'quality': Regex('^[\w]*$'), 'temp_dir': _PATH, }),
    'storage': Map({'path': _PATH, 'vod_path': _PATH, }),
    'telegram': Map({'enabled': Bool(), 'api_token': Regex('^[\w:_\-]+$'), 'chat_id': _WORD, }),
})


def load(path: str = CONFIG_FILE) -> Optional[Dict]:
    with open(path, 'rt') as file:
        return yaml_load(file.read(), schema=__CONFIG_SCHEMA).data
