...
class TwitchAPI:
    ...
    def get_stream_playlist_uri(self, channel: str, quality: str) -> str:
        last_broadcasts = self.get_videos(channel)
        # TODO: handle StopIteration due to the stream is already over (very short stream?)
        live_broadcast = next(broadcast for broadcast in last_broadcasts if broadcast['status'] == 'recording')
        if live_broadcast:
            vod_id = live_broadcast['_id'].lstrip('v')
